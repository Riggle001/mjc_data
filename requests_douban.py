#!/usr/bin/env python3
# -*- coding: utf-8 -*-
#来自https://github.com/dylanxia2017/douban_spider/blob/master/douban.py
...
def spider_movie(page_num):
    ...
    for page in range(page_num):
        ...
        url = "https://movie.douban.com/subject/27010768/reviews?start={}".format(page * 20)
        try:
    ...
    with open("用户链接.txt","r") as filehandler:
        for url_space in filehandler:
            ...
            with open("douban.cookie","rb") as f:
                ...
                with open("用户地区.txt", "w",encoding='utf-8') as f2:
                    for loc in location:
                        f2.writelines("%s\n" % loc)
...
def spider_people(page_num):
    ...
    with open("用户地区.txt", "a",encoding='utf-8') as f3:
        for loc in location:
            f3.writelines("%s\n" % loc)
...
def spider_review():
    ...
    with open("评论链接.txt","r") as filehandler:
        for url_space in filehandler:
            ...
            with open("评论集合.txt", "w",encoding='utf-8') as filehandle2:#以防乱码，需要在后面写encoding，在cont继续编码解码方法
